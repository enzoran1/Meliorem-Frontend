...
export function getAllUser(token, success,error) {
  console.log(token);
    axios.get(`${BASE_URL}/user`,getTokenHeader(token)).then(response=>{
      if (success)
        success(response.data); 
    }
    ).catch(err=>{
...
export function getUser(token,id, success,error) {
  console.log(token);
    axios.get(`${BASE_URL}/user/${id}`,getTokenHeader(token)).then(response=>{
      if (success)
        success(response.data); 
    }
    ).catch(err=>{
...
export function deleteUser(token,id, success,error) {
    axios.delete(`${BASE_URL}/user/${id}`,getTokenHeader(token)).then(response=>{
      if (success)
        success(response.data); 
    }
    ).catch(err=>{
